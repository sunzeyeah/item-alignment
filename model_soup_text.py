...
def get_parser():
    ...
    parser.add_argument("--soups", required=True, type=str, help="epochs to be used for uniform soup")
...
def load_raw_data(args):
    valid_data = []
    with open(os.path.join(args.data_dir, "finetune_train_valid.tsv"), "r", encoding="utf-8") as r:
        while True:
            line = r.readline()
            if not line:
                break
...
def load_kg_tokenizer(args):
    ...
    with open(os.path.join(args.data_dir, "entity2id.txt"), "r", encoding="utf-8") as r:
        while True:
            line = r.readline()
            if not line:
                break
    ...
    with open(os.path.join(args.data_dir, "relation2id.txt"), "r", encoding="utf-8") as r:
        while True:
            line = r.readline()
            if not line:
                break
...
def main():
    ...
    st = OrderedDict()
    soups = args.soups.split(",")
    named_parameters = dict(model.named_parameters())
    for soup in soups:
        state_dict = torch.load(args.file_state_dict.format(soup), map_location="cpu")
    ...
    logger.info(f"Finished uniform soup on epochs: {soups}")
    ...
    output_model_file = os.path.join(output_model_path, f"pkgm_finetune-uniform_soup-epoch-{args.soups}.bin")
    torch.save(st, output_model_file)
    logger.info(f"Finished saving uniform soup model")

    # load raw data
    valid_data, test_data = load_raw_data(args)
    logger.info(f"# valid samples: {len(valid_data)}, # test samples: {len(test_data)}")
